...
var placesDsf = new Dsf('places');

placesDsf.connectorMappers = {};

placesDsf.filter = function (context, oldMatches) {
	var places = [];
	$.each(context, function (connectorId, rdf) {
		var mapper = placesDsf.connectorMappers[connectorId];
		if (mapper) {
			jQuery.merge(places, mapper(rdf, oldMatches));
		}
	});
...
placesDsf.connectorMappers['rdfa'] = function (rdf) {
	 var ret = [];
	 rdf
...
placesDsf.connectorMappers['stanbol'] = function (rdf, matches) {
    var ret = [];
